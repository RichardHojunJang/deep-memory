"""Tests for the thin adapter layer."""
...
def test_tool_handler_maps_hermes_args():
    adapter = next(item for item in hermes_tools.iter_tool_adapters() if item.name == "learn")
    result = adapter.handler(
        {"entity": "Alice", "insight": "Likes SQLite", "type": "explicit"},
        task_id="task-1",
    )
    assert '"stored": true' in result.lower()
